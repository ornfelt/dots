...
function session_manager.restore_state(window)
  ...
  if not workspace_data then
    status.notify(window, 'WezTerm',
      'Workspace state file not found for workspace: ' .. workspace_name, false)
    return
  end

  if recreate_workspace(window, workspace_data) then
    status.notify(window, 'WezTerm', 'Workspace state loaded for workspace: ' .. workspace_name,
      true)
  else
    status.notify(window, 'WezTerm', 'Workspace state loading failed for workspace: ' .. workspace_name,
      false)
  end
end
...
function session_manager.save_state(window)
  status.progress(window, "Saving session: " .. window:active_workspace() .. "...")
  ...
  if save_to_json_file(data, file_path) then
    status.notify(window, 'WezTerm Session Manager', 'Workspace state saved successfully', true)
  else
    status.notify(window, 'WezTerm Session Manager', 'Failed to save workspace state', false)
  end
end
