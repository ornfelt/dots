...
-- somewm doesn't expose awful.util on the awful table; this config uses it as a shared namespace
awful.util          = awful.util or require("awful.util")
...
-- somewm composites itself, so no picom
run_once({ "xfce4-power-manager" })

-- xset (screen blanking/DPMS) is X11-only; somewm doesn't blank the screen on its own
...
beautiful.init(string.format("%sthemes/%s/theme.lua", gears.filesystem.get_configuration_dir(), chosen_theme))
...
-- Wallpaper re-set on geometry change is handled by awful.wallpaper (see theme.at_screen_connect)
...
globalkeys = mytable.join(
    ...
    -- bind mod-shift-p: toggle wibox visibility
    ...
    -- bind mod-right: awful.tag.viewnext
    ...
    -- bind mod-o: awful.tag.incmwfact +0.05 (increase master width)
    ...
    -- bind mod-shift-a: focused client opacity -5%
    awful.key({ modkey, "Shift" }, "a", function()
        local c = client.focus
        if c then c.opacity = math.max(0.05, (c.opacity or 1) - 0.05) end
    end,
        {description = "decrease transparency", group = "custom"}),

    -- bind mod-ctrl-a: focused client opacity +5%
    awful.key({ modkey, "Control" }, "a", function()
        local c = client.focus
        if c then c.opacity = math.min(1, (c.opacity or 1) + 0.05) end
    end,
...
clientkeys = mytable.join(
    ...
    -- bind mod-q: killclient (close window)
    ...
    -- bind mod-space: toggle floating and center
    awful.key(
      { modkey }, "space",
      function(c)
    ...
    -- bind mod-shift-l: move client to next screen
    ...
    -- bind mod-shift-h: move client to previous screen
    ...
    -- bind mod-shift-ctrl-m: toggle maximize
...
local function is_odd_with_mask(tag_index)
    return bit.band(binary_mask, bit.lshift(1, tag_index - 1)) ~= 0
end
...
local function is_odd(index)
  --return index % 2 == 1
  return bit.band(EVEN_TAG_MASK, bit.lshift(1, index - 1)) == 0
end
...
for i = 1, 9 do
  globalkeys = mytable.join(globalkeys,
    -- View tag only.
    -- bind mod-[1-9]: view tag on appropriate monitor
    ...
    -- bind mod-shift-[1-9]: move client to tag and view it
    ...
    -- Move client to tag.
    -- bind mod-ctrl-[1-9]: move client to tag (stay on current)
...
clientbuttons = mytable.join(
    ...
    -- bind mod-button1: move client (drag)
    awful.button({ modkey }, 1, function (c)
        c:emit_signal("request::activate", "mouse_click", {raise = true})
        awful.mouse.client.move(c)
    end),
    -- bind mod-button3: resize client (drag)
...
-- awful.rules moved to ruled.client in somewm
...
local function move_even_tag_clients()
  ...
  for _, c in ipairs(client.get()) do
    ...
    for _, tag in ipairs(tags) do
      local tag_index = tonumber(tag.name)
      if tag_index and bit.band(EVEN_TAG_MASK, bit.lshift(1, tag_index - 1)) ~= 0 then
