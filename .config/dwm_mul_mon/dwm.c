...
void sendmonview(Client *c, Monitor *m)
{
    if (c->mon == m)
        return;
    unfocus(c, 1);
...
void tagnthmonview(const Arg *arg)
{
    if (!selmon->sel || !mons->next)
        return;
