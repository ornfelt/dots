...
keydwm() {
    ...
    [ -s "$MKEYS" ] || s_err "You have no mapped keys!"
    # clean up
...
checkdwm(){
    [ ! -f "$FILE" ] && s_err "File '$FILE' is invalid!"
    case $1 in 
        markdown | md) mddwm  "$2" ;;
        status   | s)  statdwm ;;
        key      | k)  keydwm "$2" ;;
        *) s_err "You must provide an argument for the file! Type 'rice help' to get more information!"
    esac
}
FILE="$1"
case $1 in
