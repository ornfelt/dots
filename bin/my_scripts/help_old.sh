...
show_git_help() {
  ...
  if test_section_match "push clone token" "$kw"; then
  ...
  if test_section_match "log history graph decorate" "$kw"; then
    write_code_line "git log --graph --decorate                    # Display history with graph and decorate"
  fi

  if test_section_match "diff show changes latest commit" "$kw"; then
  ...
  if test_section_match "apply patch" "$kw"; then
...
show_scripts_help() {
  ...
  if test_section_match "Meta help script helper script_helper gui launcher" "$kw"; then
    printf "%bMeta:%b\n" "$DARKGRAY" "$RESET"
    write_command_with_description ".help"          "show this help"
    write_command_with_description ".script_helper" "open GUI helper for running other helper scripts"
    printf "\n"
  fi

  if test_section_match "Navigation cd helpers" "$kw"; then
  ...
  if test_section_match "Run launcher helpers" "$kw"; then
  ...
  if test_section_match "Listing helpers list" "$kw"; then
    printf "%bListing helpers:%b\n" "$DARKGRAY" "$RESET"
    write_command_with_description ".list_colors"         "print colors"
    write_command_with_description ".list_all_colors"     "print all colors"
    ...
    write_command_with_description ".proc"                "list/inspect processes"
    write_command_with_description ".lsh"                 "ls helper script"
    printf "\n"
  fi

  if test_section_match "Network helpers wifi" "$kw"; then
  ...
  if test_section_match "Search inspect dump helpers find replace" "$kw"; then
    ...
    write_command_with_description ".fr"          "find and replace"
    write_command_with_description ".fr_py"       "find and replace (Python)"
    write_command_with_description ".fr_cs"       "find and replace (C#)"
    write_command_with_description ".find_files"  "helper script for finding files"
  ...
  if test_section_match "Build tools maintenance cmake clean update diff git_diff" "$kw"; then
    printf "%bBuild / tools / maintenance:%b\n" "$DARKGRAY" "$RESET"
    write_command_with_description ".cmake"           "helper script for cmake"
    write_command_with_description ".cmake_old"       "old cmake helper script"
    write_command_with_description ".cmake_py"        "Python equivalent of cmake helper script"
    write_command_with_description ".clean"           "clean up build dirs (bin, obj, etc.)"
    write_command_with_description ".pkg"             "helper script for packages"
    write_command_with_description ".sln"             "helper script for VS solutions"
    ...
    write_command_with_description ".git_diff"        "helper script for generating Git diffs"
...
show_env_help() {
  ...
  if test_section_match "System machine hostname OS CPU" "$kw"; then
  ...
  if test_section_match "User profile home" "$kw"; then
    printf "\n"
  ...
  if test_section_match "Shell process PATH" "$kw"; then
    printf "\n"
  ...
  if test_section_match "Tool app versions version" "$kw"; then
    printf "\n"
    printf "%bTool / app versions:%b\n" "$DARKGRAY" "$RESET"
    write_command_with_description 'bash --version'         "Bash version"
    write_command_with_description 'echo $BASH_VERSION'     "Bash version string"
    write_command_with_description 'zsh --version'          "Zsh version"
    write_command_with_description 'echo $ZSH_VERSION'      "Zsh version string (inside zsh)"
    write_command_with_description 'echo $ZSH'              "Oh My Zsh install dir (if set)"
    write_command_with_description 'wezterm -V'             "WezTerm version"
    write_command_with_description 'nvim --version'         "Neovim version"
    write_command_with_description 'vim --version'          "Vim version"
    write_command_with_description 'git --version'          "Git version"
    write_command_with_description 'tmux -V'                "tmux version"
    write_command_with_description 'cmake --version'        "CMake version"
    write_command_with_description 'curl --version'         "curl version"
    write_command_with_description 'ssh -V'                 "OpenSSH version"
  fi

  if test_section_match "System info commands nproc uname lscpu" "$kw"; then
    printf "\n"
  ...
  if test_section_match "Print inspect variables echo printenv" "$kw"; then
    printf "\n"
...
show_other_help() {
  ...
  if test_section_match "keepawake vim clipboard" "$kw"; then
  ...
  if test_section_match "Bash file directory operations" "$kw"; then
    printf "\n"
    printf "%bBash file/directory operations:%b\n" "$YELLOW" "$RESET"
  fi

  if test_section_match "List only directories" "$kw"; then
    printf "\n"
  ...
  if test_section_match "List only files" "$kw"; then
    printf "\n"
  ...
  if test_section_match "Filter by keyword" "$kw"; then
    printf "\n"
  ...
  if test_section_match "View file content head tail" "$kw"; then
    printf "\n"
  ...
  if test_section_match "Count items files directories" "$kw"; then
    printf "\n"
...
show_grep_help() {
  ...
  if test_section_match "Basic Recursive Search" "$kw"; then
    printf "Basic Recursive Search\n"
    write_code_line 'grep -r "your_search_text" .'
    printf "\n"
  fi

  if test_section_match "Non-Recursive Search" "$kw"; then
    printf "Non-Recursive Search\n"
    write_code_line 'grep "your_search_text" *'
    printf "\n"
  fi

  if test_section_match "Search Only in Files with Specific Extensions" "$kw"; then
  ...
  if test_section_match "CMake CMakeLists" "$kw"; then
  ...
  if test_section_match "Context search temp file second grep" "$kw"; then
...
show_c_help() {
  ...
  if test_section_match "Version help" "$kw"; then
    printf "\n"
  ...
  if test_section_match "Compile run build" "$kw"; then
    printf "\n"
  ...
  if test_section_match "Clean project generated files" "$kw"; then
    printf "\n"
...
show_csharp_help() {
  ...
  if test_section_match "Create new projects console classlib webapi worker" "$kw"; then
    printf "\n"
  ...
  if test_section_match "Useful dotnet commands build run test" "$kw"; then
    printf "\n"
  ...
  if test_section_match "Solution files sln slnx" "$kw"; then
    printf "\n"
  ...
  if test_section_match "EF Core tools Entity Framework" "$kw"; then
    printf "\n"
  ...
  if test_section_match "Add NuGet packages" "$kw"; then
    printf "\n"
...
show_rust_help() {
  ...
  if test_section_match "Basic usage new build run" "$kw"; then
    printf "\n"
  ...
  if test_section_match "logs redirected output test.txt" "$kw"; then
    printf "\n"
  ...
  if test_section_match "Backtraces RUST_BACKTRACE" "$kw"; then
    printf "\n"
  ...
  if test_section_match "Usage with args dt flag" "$kw"; then
    printf "\n"
  ...
  if test_section_match "Feature flags features" "$kw"; then
    printf "\n"
  ...
  if test_section_match "CARGO_FEATURES env var rust-analyzer neovim" "$kw"; then
    printf "\n"
  ...
  if test_section_match "RUSTFLAGS warnings suppress" "$kw"; then
    printf "\n"
    write_code_line 'export RUSTFLAGS="-Awarnings"       # Allow/suppress all Rust compiler warnings'
  fi

  if test_section_match "Add remove packages crates" "$kw"; then
    printf "\n"
...
show_java_help() {
  ...
  if test_section_match "Add packages Maven" "$kw"; then
    printf "\n"
  ...
  if test_section_match "Add packages Gradle" "$kw"; then
    printf "\n"
...
show_python_help() {
  ...
  if test_section_match "Version help" "$kw"; then
    ...
    write_code_line "pip --version"
  fi

  if test_section_match "Basic usage run venv virtual environment" "$kw"; then
    printf "\n"
    printf "%bBasic usage:%b\n" "$YELLOW" "$RESET"
  ...
  if test_section_match "Add remove packages pip install" "$kw"; then
    printf "\n"
...
show_go_help() {
  ...
  if test_section_match "Basic usage init run build test" "$kw"; then
    printf "\n"
    printf "%bBasic usage:%b\n" "$YELLOW" "$RESET"
  ...
  if test_section_match "Build tags feature flags" "$kw"; then
    printf "\n"
  ...
  if test_section_match "Persistent GOFLAGS" "$kw"; then
    printf "\n"
  ...
  if test_section_match "output redirected test.txt" "$kw"; then
    printf "\n"
  ...
  if test_section_match "Add remove packages go get" "$kw"; then
    printf "\n"
    printf "%bAdd / remove packages:%b\n" "$YELLOW" "$RESET"
...
show_js_help() {
  ...
  if test_section_match "run init start basic usage" "$kw"; then
    printf "\n"
    printf "%bDo this:%b\n" "$YELLOW" "$RESET"
  ...
  if test_section_match "Add remove packages npm install" "$kw"; then
    printf "\n"
    printf "%bAdd / remove packages:%b\n" "$YELLOW" "$RESET"
...
show_ts_help() {
  ...
  if test_section_match "init setup build dev run basic usage" "$kw"; then
    printf "\n"
    printf "%bDo this:%b\n" "$YELLOW" "$RESET"
  ...
  if test_section_match "Add remove packages npm install types" "$kw"; then
    printf "\n"
    printf "%bAdd / remove packages:%b\n" "$YELLOW" "$RESET"
    write_code_line "npm install some-package                          # Add latest"
    write_code_line "npm install some-package@1.2.3                    # Add specific version"
...
case "$mode" in
  # Languages
  c)           show_c_help "$keyword" ;;
  csharp)      show_csharp_help "$keyword" ;;
  cpp)         show_cpp_help "$keyword" ;;
  rust)        show_rust_help "$keyword" ;;
  java)        show_java_help "$keyword" ;;
  python)      show_python_help "$keyword" ;;
  go)          show_go_help "$keyword" ;;
  javascript)  show_js_help "$keyword" ;;
  typescript)  show_ts_help "$keyword" ;;

  # Other modes
  git)         show_git_help "$keyword" ;;
  grep)        show_grep_help "$keyword" ;;
  gitgrep)     show_gitgrep_help "$keyword" ;;
  ripgrep)     show_ripgrep_help "$keyword" ;;
  scripts)     show_scripts_help "$keyword" ;;
  env)         show_env_help "$keyword" ;;
  other)       show_other_help "$keyword" ;;
